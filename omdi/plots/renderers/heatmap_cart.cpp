...
#include "plots/renderers/renderers.h"
...
namespace omdi::plots::renderers {
  ...
  struct GetterHeatmapCartRowMaj {
    ...
    IMPLOT_INLINE RectC operator()(I idx) const {
      double            val = !UseLog ? (double)Values[idx]
                                      : (double)(QLOGSCALE(Values[idx]));
      ...
      const float t = (val > ScaleMax)
                        ? 1.0f
                        : ((val <= ScaleMin)
                             ? 0.0f
                             : ImClamp((float)ImRemap01(val, ScaleMin, ScaleMax),
                                       0.0f,
                                       1.0f));
