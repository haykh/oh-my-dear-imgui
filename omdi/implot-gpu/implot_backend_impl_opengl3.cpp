...
namespace ImPlot::Backend {
  ...
  struct HeatmapShader {
    ...
    GLuint AttribLocationProjection =
      0; ///< Attribute location for the projection matrix uniform
  ...
  struct ContextData {
    ...
    GLuint AttribLocationImGuiProjection =
      0; ///< Attribute location for the projection matrix uniform (ImGui default shader)
  ...
  void DestroyContext() {
    ContextData* Context = ((ContextData*)omdi::implot::GImPlotWrapper->backendCtx);
...
#define HEATMAP_FRAGMENT_SHADER_CODE                                           \
  ...
  "\n"                                                                         \
  "    return (exp((maxrl - minrl) * x + minrl) - min_rng) / (max_rng - "      \
  "min_rng);"                                                                  \
  "}\n"                                                                        \
  "\n"                                                                         \
  "void main()\n"                                                              \
  ...
  "    float uv_x = ax_log.x ? log_den(Frag_UV.x, bounds_min.x, "              \
  "bounds_max.x) : Frag_UV.x;\n"                                               \
  "    float uv_y = ax_log.y ? log_den(Frag_UV.y, bounds_min.y, "              \
  "bounds_max.y) : Frag_UV.y;\n"                                               \
  "\n"                                                                         \
  "    float value = float(texture(heatmap, vec2(uv_x, uv_y)).r);\n"           \
  "    float offset = (value - min_val) / (max_val - min_val);\n"              \
  "          offset = mix(min_tex_offs, 1.0 - min_tex_offs, clamp(offset, "    \
  "0.0f, 1.0f));\n"                                                            \
  "    Out_Color = texture(colormap, offset);\n"                               \
  "}\n"

  static void CompileShader(HeatmapShader& ShaderProgram,
    ...
    ShaderProgram.AttribLocationProjection = glGetUniformLocation(ShaderProgram.ID,
                                                                  "ProjMtx");
    ShaderProgram.AttribLocationMinValue = glGetUniformLocation(ShaderProgram.ID,
                                                                "min_val");
    ShaderProgram.AttribLocationMaxValue = glGetUniformLocation(ShaderProgram.ID,
                                                                "max_val");
    ShaderProgram.AttribLocationMinBounds = glGetUniformLocation(ShaderProgram.ID,
                                                                 "bounds_min");
    ShaderProgram.AttribLocationMaxBounds = glGetUniformLocation(ShaderProgram.ID,
                                                                 "bounds_max");
    ShaderProgram.AttribLocationAxisLog = glGetUniformLocation(ShaderProgram.ID,
                                                               "ax_log");

    glUseProgram(ShaderProgram.ID);
    glUniform1i(glGetUniformLocation(ShaderProgram.ID, "heatmap"),
                0); // Set texture slot of heatmap texture
    glUniform1i(glGetUniformLocation(ShaderProgram.ID, "colormap"),
  ...
  static void CreateHeatmapShader(const ImDrawList*, const ImDrawCmd*) {
    ...
    Context.AttribLocationImGuiProjection = glGetUniformLocation(Context.ImGuiShader,
                                                                 "ProjMtx");
    GLuint AttribLocationVtxPos = (GLuint)glGetAttribLocation(Context.ImGuiShader,
                                                              "Position");
    GLuint AttribLocationVtxUV = (GLuint)glGetAttribLocation(Context.ImGuiShader,
                                                             "UV");

    GLchar* VertexShaderCode   = new GLchar[1000];
    GLchar* FragmentShaderCode = new GLchar[1000];

    snprintf(VertexShaderCode,
             1000,
             HEATMAP_VERTEX_SHADER_CODE,
  ...
  static void RenderCallback(const ImDrawList*, const ImDrawCmd* cmd) {
    ...
    glGetUniformfv(Context.ImGuiShader,
                   Context.AttribLocationImGuiProjection,
                   &OrthoProjection[0][0]);
    ...
    glBindTexture(GL_TEXTURE_2D, data.HeatmapTexID); // Set texture ID of data
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_1D, data.ColormapTexID); // Set texture ID of colormap

    glUniformMatrix4fv(data.ShaderProgram->AttribLocationProjection,
                       1,
                       GL_FALSE,
                       &OrthoProjection[0][0]);
    glUniform1f(data.ShaderProgram->AttribLocationMinValue,
                data.MinValue); // Set minimum range
    glUniform1f(data.ShaderProgram->AttribLocationMaxValue,
                data.MaxValue); // Set maximum range
    glUniform2i(data.ShaderProgram->AttribLocationAxisLog,
                data.AxisLogX,
                data.AxisLogY); // Logarithmic axis
    glUniform2f(data.ShaderProgram->AttribLocationMinBounds,
                (float)data.MinBounds.x,
                (float)data.MinBounds.y); // Set minimum bounds
    glUniform2f(data.ShaderProgram->AttribLocationMaxBounds,
                (float)data.MaxBounds.x,
                (float)data.MaxBounds.y); // Set maximum bounds
  }
  ...
  static void SetTextureData(GLuint      textureID,
                             const void* data,
                             GLsizei     rows,
                             GLsizei     cols,
  ...
  void SetHeatmapData(GLuint textureID, const ImS64* values, int rows, int cols) {
    ...
    SetTextureData(textureID,
                   Context.TempS32.Data,
                   rows,
                   cols,
                   GL_R32I,
                   GL_RED_INTEGER,
  ...
  void RenderHeatmap(int                itemID,
    ...
    data.ShaderProgram = (data_type == ImGuiDataType_Float ||
    ...
    switch (data_type) {
      case ImGuiDataType_S8:
        SetTextureData(data.HeatmapTexID,
                       (const ImS8*)values,
                       rows,
                       cols,
                       GL_R8I,
      ...
      case ImGuiDataType_S64:
        SetHeatmapData(data.HeatmapTexID, (const ImS64*)values, rows, cols);
        break;
      case ImGuiDataType_U64:
        SetHeatmapData(data.HeatmapTexID, (const ImU64*)values, rows, cols);
        break;
      ...
      case ImGuiDataType_Double:
        SetHeatmapData(data.HeatmapTexID, (double*)values, rows, cols);
        break;
    };
    ...
    DrawList.PrimRectUV(coords_min,
                        coords_max,
                        ImVec2(0.0f, reverse_y ? 1.0f : 0.0f),
                        ImVec2(1.0f, reverse_y ? 0.0f : 1.0f),
                        0);
