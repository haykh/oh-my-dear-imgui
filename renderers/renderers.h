#ifndef PLOTS_RENDERERS_RENDERERS_H
#define PLOTS_RENDERERS_RENDERERS_H

#include "plots/renderers/transformers.h"
...
namespace omdi::plots::renderers {
  ...
  IMPLOT_INLINE void PrimArcFill(ImDrawList&   draw_list,
    ...
    for (int i { 0 }; i < ntheta; ++i) {
      draw_list._IdxWritePtr[n + 0] = (ImDrawIdx)(draw_list._VtxCurrentIdx) +
                                      (i + 1);
      draw_list._IdxWritePtr[n + 1] = (ImDrawIdx)(draw_list._VtxCurrentIdx) +
                                      2 * ntheta + 1 - i;
      if (i == 0) {
      ...
      draw_list._IdxWritePtr[n + 3] = (ImDrawIdx)(draw_list._VtxCurrentIdx) +
                                      (i + 1);
      draw_list._IdxWritePtr[n + 4] = (ImDrawIdx)(draw_list._VtxCurrentIdx) +
                                      (i + 2);
      draw_list._IdxWritePtr[n + 5] = (ImDrawIdx)(draw_list._VtxCurrentIdx) +
                                      2 * ntheta + 1 - i;
      n += 6;
    }
  ...
  struct RendererArcC : RendererBase {
    ...
    RendererArcC(const _Getter& getter)
      : RendererBase(getter.Count, 6 * NTheta, 2 * (NTheta + 1))
      , Getter(getter) {}
...
#endif // PLOTS_RENDERERS_RENDERERS_H
